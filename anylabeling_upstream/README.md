...
## 🚀 Improvements & Features in this Fork (`guillermopetcho/anylabeling--ama-gi`)

- **⚡ Memory Race Condition & Stability Fixes**:
  - Decoupled `QImage` memory from NumPy arrays (`rgb_view.copy()`) to fix crashes during rapid image switching in Q(++) mode.
  - Thread-safe ONNX Runtime inference using `threading.Lock` between GUI and background pre-loader threads.
  - Installed a custom `sys.excepthook` to log uncaught Qt slot exceptions and prevent application crashes.
- **📊 Progress Bar & Cancel Button ("❌ Cancelar")**:
  - Real-time progress bar tracking Hugging Face model downloads (`%`) and inference status.
  - Added a **"❌ Cancelar"** button to stop model loading or inference at any time.
- **🧠 Minimized Panel Segmentation**:
  - Minimize or hide the Auto-Labeling panel (brain icon) while maintaining active prompt/point (+) modes for uninterrupted canvas labeling.
- **📁 Multi-Image & Annotation Folder Transfer ("📁 Cambiar de Carpeta...")**:
  - Context menu option to move single or multiple selected images together with their corresponding `.json` annotation files to a target directory.
  - Live modal progress dialog (`QProgressDialog`) with real-time percentage, event processing, and cancellation support.
  - Fully compatible with label filtering and multi-selection (`ExtendedSelection`).
- **🎨 Mini Image Editor & Dataset Augmentation**:
  - Adjust brightness, contrast, saturation, sharpness, and perform dataset augmentations directly in the application.
- **🔍 Dataset Inspector & Global Label Management**:
  - Inspect class statistics, polygon counts, and perform global class label replacement across the dataset.
