...
class Canvas(QtWidgets.QWidget):  # pylint: disable=too-many-public-methods, too-many-instance-attributes
    ...
    # QT Overload
    def event(self, ev):
        """QT Overload: Consumes all input events when canvas is loading"""
    ...
    def mousePressEvent(self, ev):
        ...
        # Si el modo Q(++) está activo en el widget padre, asegurar que el canvas esté en auto-labeling
        if (
            hasattr(self, "parent")
            and getattr(self.parent, "q_plus_plus_mode", False)
            and hasattr(self.parent, "auto_labeling_widget")
            and self.parent.auto_labeling_widget
            and not self.drawing()
        ):
            if (
                not self.is_auto_labeling
                or self.auto_labeling_mode == AutoLabelingMode.NONE
            ):
                self.parent.auto_labeling_widget.set_auto_labeling_mode(
    ...
    def mouseReleaseEvent(self, ev):
        ...
        if self.moving_shape and self.h_hape:
            try:
                ...
                if self.shapes_backups and self.shapes_backups[-1][index].points != self.shapes[index].points:
                    self.store_shapes()
    ...
    def end_move(self, copy):
        """End of move"""
        if not self.selected_shapes or not self.selected_shapes_copy:
            return False
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        if self.is_loading or self.is_auto_labeling:
            return
    ...
    def merge_selected_polygons(self):
        """Merge selected polygon shapes into a single polygon shape"""
        ...
        # Filter shapes with points
        shapes_to_merge = [s for s in self.selected_shapes if len(s.points) >= 3]
        if len(shapes_to_merge) < 2:
            return None

        # Find overall bounding box
        all_pts = []
        for s in shapes_to_merge:
            for p in s.points:
                all_pts.append([int(p.x()), int(p.y())])
        ...
        for s in shapes_to_merge:
            poly_pts = np.array(
                [[int(p.x() - min_x), int(p.y() - min_y)] for p in s.points],
                dtype=np.int32,
            )
        ...
        merged_pts = approx.reshape(-1, 2)
        if len(merged_pts) < 3:
            return None

        # Create new merged shape using properties of first shape
        first_shape = shapes_to_merge[0]
        merged_shape = Shape(
            label=first_shape.label,
            shape_type="polygon",
            flags=copy.deepcopy(first_shape.flags),
        )
        for pt in merged_pts:
            merged_shape.add_point(
                QtCore.QPointF(float(pt[0] + min_x), float(pt[1] + min_y))
            )
        merged_shape.closed = True
        merged_shape.fill_color = first_shape.fill_color
        merged_shape.line_color = first_shape.line_color

        # Replace merged shapes in self.shapes
        for s in shapes_to_merge:
            if s in self.shapes:
                self.shapes.remove(s)

        self.shapes.append(merged_shape)
        self.selected_shapes = [merged_shape]
        self.store_shapes()
        self.new_shape.emit()
        self.update()
        return merged_shape

    def convert_to_rectangles(self):
        """Convert selected polygon shapes (or all polygons) into rectangle bounding boxes"""
        target_shapes = self.selected_shapes if self.selected_shapes else self.shapes
        polygons = [s for s in target_shapes if s.shape_type == "polygon" and len(s.points) >= 3]
        if not polygons:
            return 0
    ...
    def convert_to_polygons(self):
        """Convert selected rectangle shapes (or all rectangles) into 4-point polygons"""
        target_shapes = self.selected_shapes if self.selected_shapes else self.shapes
        rectangles = [s for s in target_shapes if s.shape_type == "rectangle" and len(s.points) == 2]
        if not rectangles:
            return 0
    ...
    def finalise(self):
        ...
        if self.is_loading or self.current is None:
            return
        if self.is_auto_labeling and self.auto_labeling_mode != AutoLabelingMode.NONE:
    ...
    def update_auto_labeling_marks(self):
        ...
        if marks:
            self.set_loading(True, self.tr("Inferenciando modelo de IA..."))
    ...
    def keyPressEvent(self, ev):
        ...
        if self.drawing():
            if key == QtCore.Qt.Key.Key_Return and self.can_close_shape():
                self.finalise()
