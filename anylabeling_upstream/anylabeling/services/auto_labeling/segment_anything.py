...
class SegmentAnything(Model):
    ...
    def __init__(self, config_path, on_message) -> None:
        ...
        # Cache for image embedding (reduced size to prevent RAM bloat / system freezing)
        self.cache_size = 3
    ...
    def set_auto_labeling_marks(self, marks):
        """Set auto labeling marks (thread-safely via deepcopy)."""
        self.marks = copy.deepcopy(marks) if marks else []
    ...
    def post_process(self, masks, label="AUTOLABEL_OBJECT"):
        ...
        try:
            ...
            print(f"Warning: Exception in post_process mask preparation: {e}")
            return []

        # Refine contours with optimized epsilon to prevent excessive vertex lag
        approx_contours = []
        for contour in contours:
            epsilon = 0.002 * cv2.arcLength(contour, True)
            approx = cv2.approxPolyDP(contour, epsilon, True)
            if len(approx) >= 3:  # Only keep valid polygons
    ...
    def predict_shapes(self, image, filename=None) -> AutoLabelingResult:
        ...
        """
        self.stop_inference = False
        # Thread-safe snapshot copy of marks at entry to prevent race condition crashes
        # if main GUI thread mutates self.marks during background worker execution
        current_marks = copy.deepcopy(self.marks) if self.marks else []

        if image is None or (not current_marks and self.prompt_mode != "text"):
            return AutoLabelingResult([], replace=False, filename=filename)

        shapes = []
        try:
            with self.onnx_lock:
                # Use cached image embedding if possible (key includes dimensions for rotated images)
                cache_key = (filename, image.width(), image.height()) if (image and hasattr(image, "width")) else filename
                ...
                if self._is_sam3:
                    ...
                    ] if self.prompt_mode == "text" else [self.text_prompt or "visual"]

                    inference_marks = current_marks if self.prompt_mode != "text" else []
        ...
        result = AutoLabelingResult(shapes, replace=False, filename=filename)
        return result

    def stop_pre_inference(self):
        """Safely stop background pre-inference thread and release resources."""
        self.stop_inference = True
        if self.pre_inference_thread and self.pre_inference_thread.isRunning():
            try:
                self.pre_inference_thread.quit()
                if not self.pre_inference_thread.wait(500):
                    self.pre_inference_thread.terminate()
                    self.pre_inference_thread.wait(200)
            except Exception:
                pass
            finally:
                self.pre_inference_thread = None
    ...
    def preload_worker(self, files):
        """
        Preload next files, run inference and cache results safely
        """
        if self.stop_inference:
            return
        files = files[: self.preloaded_size]
        for filename in files:
            if not filename or self.stop_inference:
                return

            image = self.load_image_from_filename(filename)
            if image is None or image.isNull() or self.stop_inference:
                continue

            # Match exact cache_key format used in predict_shapes()
            cache_key = (filename, image.width(), image.height()) if hasattr(image, "width") else filename
            if self.image_embedding_cache.find(cache_key):
                continue

            try:
                cv_image = qt_img_to_rgb_cv_img(image, filename)
                if cv_image is None or self.stop_inference:
                    return
                ...
                if not self.stop_inference and image_embedding is not None:
                    self.image_embedding_cache.put(
                        cache_key,
                        image_embedding,
                    )
            except Exception as e:
                logging.warning(f"Error in preload_worker: {e}")

    def on_next_files_changed(self, next_files):
        """
        Handle next files changed. Safely stop existing pre-inference thread first
        before starting preloading for new adjacent image.
        """
        self.stop_pre_inference()
