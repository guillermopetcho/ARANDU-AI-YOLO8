...
## 8. Las 3 Capas del Pipeline

````carousel
### Capa 1: Auto-Supervisión (MoCo v3)
```
Imágenes crudas (sin etiquetas)
        ↓
Multi-Crop Augmentation (2 globales + 5 locales)
        ↓
ConvNeXt V2 Tiny → Projector 768→512 → Predictor
        ↓
InfoNCE Simétrica + Queue de 32K negativos
        ↓
AAC v3 (PID 3-eje: τ, momentum, LR)
        ↓
moco_encoder_ready.pth (512-dim embeddings)
```
**Objetivo**: Aprender representaciones texturales de hojas de soja sin ninguna etiqueta.
<!-- slide -->
### Capa 2: Detección (YOLO26)
```
moco_encoder_ready.pth
        ↓
AranduBackbone (ConvNeXt + 4 SpatialFeatureAdapters)
        ↓
PAN-FPN Neck (P2→P5, 4 escalas)
        ↓
Detect Head (5 clases)
        ↓
Curriculum: A(adapters) → B(+P5) → C(+P4) → D(full)
```
**Objetivo**: Detectar bounding boxes de enfermedades usando el conocimiento textural del MoCo. **Requiere dataset etiquetado en formato YOLO**.
<!-- slide -->
### Capa 3: Segmentación Autodidacta (Nuevo)
```
moco_encoder_ready.pth (congelado)
        ↓
AranduBackbone → Features P3 (256ch, H/8, W/8)
        ↓
AutodidactSegmenter (Conv → BN → ReLU → Conv1×1)
        ↓
Reward Loss: max(similitud intra-cluster en espacio MoCo)
        ↓
Mapa de segmentación (K clusters)
```
**Objetivo**: Segmentar zonas de enfermedad **sin etiquetas** usando coherencia textural en el espacio latente.
````
